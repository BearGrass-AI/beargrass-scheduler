...
# Addresses are the maintainer's own test inboxes. Built messages carry the Authentication-Results pass the platform adds
...
-----Original Message-----
From: Beargrass Scheduler for Sam
...
Von: Beargrass Scheduler for Sam
...
msg /tmp/bs-r4.eml "Sam Ridge <$ORG>" "$A, $B, $Z" "$MEET" "Re: Introductions" "r4-$RUN@beargrassai.com" "$ROOT $K later-$RUN@example.org" "Thu Sep 24, 10:30am to noon
Mon Sep 28, 9am to 5pm

> On Sep 16, 2026, at 8:50 PM, Beargrass Scheduler for Sam <$SCHED> wrote:
> Tue Sep 22, 2pm to 5pm"
inject $ORG $MEET /tmp/bs-r4.eml
expect 14 "Sam's receipt with the reply-all note + the invite"
...
msg /tmp/bs-x1.eml "Sam Ridge <$ORG>" "$A" "$MEET" "Re: Introductions" "x1-$RUN@beargrassai.com" "$ROOT $K" "Sounds good, looking forward to it."
...
msg /tmp/bs-k2.eml "Sam Ridge <$ORG>" "Erin <$A>" "$MEET" "Coffee" "kickoff2-$RUN@beargrassai.com" "" "Let's find a time."
...
msg /tmp/bs-r7.eml "Sam Ridge <$ORG>" "$S2" "" "Re: Coffee" "r7-$RUN@beargrassai.com" "kickoff2-$RUN@beargrassai.com" "Tue Sep 22, 3pm to 4pm"
inject $ORG "$S2" /tmp/bs-r7.eml
expect 23 "Sam's receipt + the invite"; saw "Tuesday, September 22, 3:00 PM to 3:30 PM Mountain" "the earliest half hour inside both"
...
msg /tmp/bs-k3.eml "Sam Ridge <$ORG>" "Erin <$A>" "$MEET" "Re: Coffee" "kickoff3-$RUN@beargrassai.com" "kickoff2-$RUN@beargrassai.com" "> Second try, quoted the way Apple Mail sends a scripted body.
> 
> Length: 30 minutes
