...
Four calendars that can't see each other. One email. One invite. The organiser writes the email they were
...
When the last one is in, one calendar invite lands on every calendar, from the organiser, for the earliest
...
- **Who can start one:** an email whose From is on an invited domain, authenticated by the platform for
  exactly that domain. The envelope sender alone is never trusted. One organiser may start 10 meetings a day,
  40 a month.
- **How people answer:** by handing the email to their assistant (the email ends with a JSON contract written
  ...
  the first and the whole list is echoed back. "none" is recorded, the organiser is told, and the poll stays
  open. A reply from an address that matches nobody is reported to the organiser once and never written to.
- **The invite is the organiser's.** ORGANIZER is their address, SENT-BY the Scheduler's, so accepts reach their
...
3d the organiser answers by Reply-All: receipt + the invite (reply-all note, epoch line and invite.ics asserted)
...
(handled); a kickoff sent as a reply quotes only the organiser's words (handled).
